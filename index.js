...
function indekseGoreFenomen(dizi,index) {
  const fenomen =dizi[index];
  const metin = `${index}. indekste bulunan fenomen: ${fenomen.profile}`;
  return metin;
}
...
//console.log(profiListesi(fenomenler));
...
// const yenidizi = indekseGoreFenomen(fenomenler,3)
...
return yenidizi;
 ...
 //const yenidizi =fenomenEkle(fenomenler, 6, "Workintech", 10000000, 2022, "Instagram")
// console.log(yenidizi);
...
function enFenomenler(dizi) {
  let yenidizi = [];
  for(let key in dizi){
    if(dizi[key]["Followers"]>100000000){
      yenidizi.push(dizi[key].profile);
    }
  } 
  return yenidizi;
}
...
function fenomenGonderimSayisi(dizi, name){
  for(let i=0; i<dizi.length; i++){
    const fenomen = dizi[i];
    if(fenomen.profile === name){
...
function platformaGoreCokGonderiYapanFenomen(dizi, platformadı){
let result = ""
let maxpost = 0;
for(let i=0; i<dizi.length; i++){
  if(dizi[i].platform === platformadı){
    if(dizi[i].posts > maxpost){
      maxpost = dizi[i].posts;
